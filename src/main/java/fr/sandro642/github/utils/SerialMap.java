...
public class SerialMap {
    ...
    public HashMap<String, Object> loadData(String fileName, Plugin plugin) {
        try {
            ...
            if (!file.exists()) {
                ConnectorAPI.Logger().ERROR("ERREUR: Le fichier n'existe pas!");
                return new HashMap<>();
            }
            ...
            ConnectorAPI.Logger().ERROR("✗ Fichier introuvable: " + fileName);
            e.printStackTrace();
            return new HashMap<>();
        } catch (IOException e) {
            ConnectorAPI.Logger().ERROR("✗ Erreur de lecture du fichier: " + fileName);
            e.printStackTrace();
            return new HashMap<>();
        } catch (ClassNotFoundException e) {
            ConnectorAPI.Logger().ERROR("✗ Erreur de désérialisation: " + fileName);
            e.printStackTrace();
            return new HashMap<>();
        } catch (Exception e) {
            ConnectorAPI.Logger().ERROR("✗ Erreur générale lors du chargement: " + fileName);
            e.printStackTrace();
            return new HashMap<>();
        }
    }
